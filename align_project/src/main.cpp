...
Matrix<double> parse_kernel(string kernel)
{
    ...
    uint row = 0, col = 0;
    std::string sub_string = kernel.substr(0);
    double number = std::stod(sub_string, &pos);
    if ((sub_string[pos] == ',') || (sub_string[pos] == ';')) {
            ++pos;
    }
    numbers[row][col++] = number;
    while (row < size) {
        sub_string = sub_string.substr(pos);
        number = std::stod(sub_string, &pos);
        if ((sub_string[pos] == ',') || (sub_string[pos] == ';')) {
            ++pos;
        }
        numbers[row][col++] = number;
    ...
    Matrix<double> custom_kernel(size, size);
    for (uint i = 0; i < size; ++i) {
        for (uint k = 0; k < size; ++k) {
            custom_kernel(i, k) = numbers[i][k];
        }
    }
